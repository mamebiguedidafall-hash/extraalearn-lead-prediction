...
# Base URL of the Flask backend (the container name on the Docker network)
...
if st.button("Predict", type='primary'):
    ...
    if response.status_code == 200:
        ...
        label = "Likely to convert" if predicted_value == 1 else "Unlikely to convert"
        st.success(f"Predicted Lead Conversion: {predicted_value} ({label})")
    else:
